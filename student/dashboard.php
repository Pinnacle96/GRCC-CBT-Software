            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold"><?php echo count(array_filter($active_exams, function($exam) { return $exam['exam_status'] === 'active'; })); ?></p>
                    </div>
                    <div class="w-12 h-12 gradient-primary rounded-full flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <?php foreach ($active_exams as $exam): ?>
                        <div class="bg-white rounded-xl shadow-md overflow-hidden">
                            <div class="p-6">
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p class="text-gray-700 mb-2"><?php echo htmlspecialchars($exam['course_code']); ?> - <?php echo htmlspecialchars($exam['course_name']); ?></p>
                                    </div>
                                ...
                                <div class="mt-4 grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="font-medium"><?php echo date('M d, Y h:i A', strtotime($exam['start_time'])); ?></p>
                                    </div>
                                    <div>
                                        <p class="text-gray-500 text-sm">End Time</p>
                                        <p class="font-medium"><?php echo date('M d, Y h:i A', strtotime($exam['end_time'])); ?></p>
                                    </div>
                                    <div>
                            ...
                            <div class="px-6 py-4 bg-gray-50 border-t">
                                <?php if ($exam['exam_status'] === 'active'): ?>
                                    <?php if ($exam['session_status'] === 'in_progress'): ?>
                                        <a href="exam.php?id=<?php echo $exam['id']; ?>" class="w-full gradient-primary text-white py-2 px-4 rounded-xl font-bold hover:opacity-90 transition text-center block">Continue Exam</a>
                                    <?php elseif ($exam['session_status'] === 'completed'): ?>
                                        <a href="results.php?exam_id=<?php echo $exam['id']; ?>" class="w-full bg-gray-200 text-gray-700 py-2 px-4 rounded-xl font-bold hover:bg-gray-300 transition text-center block">View Results</a>
                                    <?php else: ?>
                                        <a href="exam.php?id=<?php echo $exam['id']; ?>" class="w-full gradient-primary text-white py-2 px-4 rounded-xl font-bold hover:opacity-90 transition text-center block">Start Exam</a>
                                    <?php endif; ?>
                                <?php elseif ($exam['exam_status'] === 'upcoming'): ?>
                                    <button disabled class="w-full bg-gray-200 text-gray-700 py-2 px-4 rounded-xl font-bold cursor-not-allowed text-center block">Not Yet Available</button>
                                <?php else: ?>
                                    <button disabled class="w-full bg-gray-200 text-gray-700 py-2 px-4 rounded-xl font-bold cursor-not-allowed text-center block">Expired</button>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                            ...
                            <?php foreach ($recent_results as $index => $result): ?>
                                <tr class="<?php echo $index % 2 === 0 ? 'bg-white' : 'bg-gray-50'; ?>">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($result['course_code']); ?></div>
                                        <div class="text-sm text-gray-500"><?php echo htmlspecialchars($result['course_name']); ?></div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900"><?php echo number_format($result['score'], 2); ?>%</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="results.php?id=<?php echo $result['id']; ?>" class="text-blue-600 hover:text-blue-900">View Details</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
